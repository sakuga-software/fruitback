...
6. Add your store to `apps/worker/src/store-conformance.test.ts`, the suite every store passes: a seed
   ...
   another, an unknown state drawn as `seeded`, replies oldest first, and an outage answered as
   `502 store-unavailable`. A remote store runs against a double of the service that keeps what it
   receives.
