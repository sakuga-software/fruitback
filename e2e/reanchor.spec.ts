...
test('pins survive a redeploy that rewrites classes, ids and the order of the page', async ({ page }) => {
  ...
  await expect(pinFor(page, 'Le bouton Ajouter')).toHaveAttribute('data-fruit-strategy', 'selector');
});
...
test('an element that is gone never leaves a pin that claims to be sure', async ({ page }) => {
  ...
  await expect(pin).toHaveAttribute('data-fruit-confident', 'false');
...
test('a note whose element is gone lands in the detached list, and one merely moved does not', async ({ page }) => {
  ...
  await expect(page.locator('[data-fruit-orphans]')).toBeHidden();
  ...
  const drawer = page.locator('[data-fruit-orphans]');
  await expect(drawer).toBeVisible();
  await expect(drawer.locator('.fruit-orphans-toggle')).toHaveText(/1 note détachée/);

  await drawer.locator('.fruit-orphans-toggle').click();
  await expect(drawer.locator('.fruit-orphans-item')).toHaveCount(1);
  await expect(drawer.locator('.fruit-orphans-note')).toContainText('Sur un champ qui va disparaître');

  // And clicking it opens that note, which is the only way left to read it on this page.
  await drawer.locator('.fruit-orphans-note').click();
  await expect(page.locator('[data-fruit-thread]')).toHaveCount(1);
  await expect(page.locator('.fruit-thread-note')).toHaveText('Sur un champ qui va disparaître');
});
