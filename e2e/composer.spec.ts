...
test('a note goes from planting to harvested, and the pin lands', async ({ page }) => {
  ...
  await expect(page.locator('[data-fruit-composer]')).toContainText(/récolté/);
  await expect(page.locator('[data-fruit-pin]')).toHaveCount(1);
  // And it closes itself once the confirmation has been read.
  await expect(page.locator('[data-fruit-composer]')).toBeHidden({ timeout: 5_000 });
});
...
test('a failed send keeps the note and stays open', async ({ page }) => {
  ...
  await expect(page.locator('[data-fruit-composer]')).toContainText(/pas passé/);
  await expect(page.locator('[data-fruit-composer]')).toBeVisible();
...
test('on a phone it is a sheet at the bottom, not a popover beside the element', async ({ page }) => {
  ...
  const measure = () =>
    page.locator('[data-fruit-composer]').evaluate((node) => {
...
test('it stays inside the viewport when the element is against the right edge', async ({ page }) => {
  ...
  await expect
    .poll(async () =>
      page.locator('[data-fruit-composer]').evaluate((node) => Math.round(node.getBoundingClientRect().right)),
    )
...
test('the pin is a drop, and it says what it is to a screen reader', async ({ page }) => {
  ...
  const badge = page.locator('.fruit-pin-badge');
