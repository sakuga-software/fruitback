...
test('the page underneath stays clickable, and the badge opens the thread', async ({ page }) => {
  ...
  await cta.click();
  await expect(page.locator('[data-fruit-thread]')).toHaveCount(0);

  await badgeFor(page, 'Le CTA devrait').click();
  const thread = page.locator('[data-fruit-thread]');
...
test('a pin that was placed rather than recognised warns whoever opens it', async ({ page }) => {
  ...
  await expect(pinFor(page, 'Sur une carte')).toHaveAttribute('data-fruit-confident', 'false');
  await badgeFor(page, 'Sur une carte').click();
  // Not "here is your feedback": "the page moved, check this one".
  await expect(page.locator('[data-fruit-thread]')).toContainText(/position|introuvable/i);
});
...
test('the colour of a pin is the Linear state, and nothing the widget decided', async ({ page }) => {
  ...
  const stages = await page.locator('[data-fruit-pin]').evaluateAll((pins) =>
    pins.map((pin) => (pin as HTMLElement).dataset.fruitStage),
  );
...
test('the team’s replies show up inside the pin', async ({ page }) => {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    const replies = page.locator('.fruit-thread-reply-body');
    if ((await replies.count()) > 0) {
      ...
      await expect(page.locator('.fruit-thread-reply-who').first()).toContainText('Alice');

      return;
    }

    // The pin that got no reply says so, which is a different thing from saying nothing.
    await expect(page.locator('.fruit-thread-empty')).toHaveText(/Pas encore de réponse/);
    await page.keyboard.press('Escape');
  }
...
test('a reply is not drawn as a bullet point', async ({ page }) => {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    const reply = page.locator('.fruit-thread-reply').first();
