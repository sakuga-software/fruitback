...
describe('createConfigPanel', () => {
  it('starts closed, because the widget is not a settings screen', () => {
    ...
    assert.equal((page.document.querySelector('[data-fb-config]') as HTMLElement).hidden, true);
  ...
  it('closes from its own button', () => {
    ...
    (page.document.querySelector('.fb-config-close') as HTMLElement).click();
  ...
  it('takes its DOM and its stylesheet with it when destroyed', () => {
    ...
    assert.equal(page.document.querySelector('[data-fb-config]'), null);
