...
describe('createCaptureHost', () => {
  it('puts everything it draws inside a Shadow root', () => {
    ...
    assert.ok(container.shadowRoot.querySelector('[data-fb-host-launch]'), 'the launch button leaked out');
    // Nothing of ours in the page's own tree beyond the single empty container.
    assert.equal(page.document.querySelector('.fb-launch'), null);
    assert.equal(page.document.querySelector('.fb-highlight'), null);
  });
  ...
  it('refuses to point at its own UI', async () => {
    ...
    const { page, engine } = mount(() => host?.root.querySelector('[data-fb-host-launch]') ?? null);
    host?.start();
