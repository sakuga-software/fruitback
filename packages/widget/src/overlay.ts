...
function buildPin(document: Document, issue: SeedIssue, resolution: AnchorResolution): HTMLElement {
  ...
  pin.style.setProperty('--fb-pin-color', stageToken(issue.stage));
...
function buildThread(document: Document, issue: SeedIssue, resolution: AnchorResolution): HTMLElement {
  ...
  thread.style.setProperty('--fb-pin-color', stageToken(issue.stage));
...
.fb-pin-badge {
  ...
  animation: fb-pin-drop var(--fb-duration-slow) cubic-bezier(0.2, 1.4, 0.35, 1);
}
.fb-pin-glyph { transform: rotate(45deg); font-size: 13px; line-height: 1; }
.fb-pin-badge:hover { filter: brightness(1.06); }
.fb-pin-badge:focus-visible { outline: 2px solid var(--fb-color-text); outline-offset: 2px; }
...
.fb-thread-close { border: 0; background: none; font-size: 18px; line-height: 1; cursor: pointer; color: var(--fb-color-text-muted); }
.fb-thread-note { margin: 8px 0 0; white-space: pre-wrap; }
.fb-thread-meta { margin: 8px 0 0; font-size: 12px; color: var(--fb-color-text-muted); }
.fb-thread-orphan { margin: 8px 0 0; font-size: 12px; color: var(--fb-color-warning); }
.fb-thread-empty { margin: 8px 0 0; font-size: 12px; color: var(--fb-color-text-subtle); font-style: italic; }
.fb-thread-replies {
  margin: 10px 0 0;
  padding: 0 0 0 10px;
  border-left: 2px solid var(--fb-color-border);
  list-style: none;
...
.fb-thread-reply-who { display: block; font-size: 11px; color: var(--fb-color-text-muted); }
.fb-thread-reply-body { margin: 2px 0 0; font-size: 12px; white-space: pre-wrap; }
.fb-thread-link { display: inline-block; margin-top: 10px; font-size: 13px; color: var(--fb-color-accent); }
`;
