...
describe('createOverlay', () => {
  it('draws a pin over the element, in document coordinates', () => {
    ...
    const pin = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
    ...
    assert.equal(pin.dataset.fruitStrategy, 'selector');
  });
  ...
  it('colours the pin by the Linear state, not by anything it stores itself', () => {
    ...
    const pin = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
    assert.equal(pin.dataset.fruitStage, 'ripe');
  ...
  it('places a pin whose element is gone at its remembered position, and says so', () => {
    ...
    const pin = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
    assert.equal(pin.dataset.fruitStrategy, 'orphan');
    assert.ok(pin.className.includes('fruit-pin-orphan'));
  ...
  it('marks a pin it placed by position rather than recognised', () => {
    ...
    const pin = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
    assert.equal(pin.dataset.fruitStrategy, 'bounds');
    assert.equal(pin.dataset.fruitConfident, 'false');
    assert.ok(pin.className.includes('fruit-pin-uncertain'));
    assert.match(page.document.querySelector('.fruit-pin-badge')?.textContent ?? '', /≈/);

    (page.document.querySelector('.fruit-pin-badge') as HTMLElement).click();
    assert.match(page.document.querySelector('[data-fruit-thread]')?.textContent ?? '', /par sa position/i);
  });
  ...
  it('re-measures when the page moves under it', () => {
    ...
    assert.equal((page.document.querySelector('[data-fruit-pin]') as HTMLElement).style.top, '460px');
  });
  ...
  it('falls back to the remembered box when the element is torn out of the page', () => {
    ...
    assert.equal(pin.style.left, '100px');
    assert.equal(pin.style.top, '200px');
    assert.ok(pin.className.includes('fruit-pin-orphan'), 'the pin should show that it lost its element');
  });
  ...
  it('does not close the thread when the click is inside it', () => {
    ...
    (page.document.querySelector('.fruit-thread-note') as HTMLElement).click();

    assert.ok(page.document.querySelector('[data-fruit-thread]'), 'the thread closed under its own click');
  });
  ...
  it('lets clicks through to the page, except on the badge', () => {
    ...
    const pin = page.document.querySelector('.fruit-pin') as HTMLElement;
    const badge = page.document.querySelector('.fruit-pin-badge') as HTMLElement;
  ...
  it('opens the thread on the badge: the note, the state and the way to Linear', () => {
    ...
    const thread = page.document.querySelector('[data-fruit-thread]');
  ...
  it('warns in the thread when the pin is only a remembered position', () => {
    ...
    assert.match(page.document.querySelector('[data-fruit-thread]')?.textContent ?? '', /introuvable/i);
  });
  ...
  it('closes the thread on Escape', () => {
    ...
    assert.equal(page.document.querySelector('[data-fruit-thread]'), null);
  });
  ...
  it('takes its own DOM with it when destroyed', () => {
    ...
    assert.equal(page.document.querySelector('[data-fruit-pin]'), null);
...
describe('the page changing underneath', () => {
  ...
  it('re-resolves its pins when the page swaps a subtree, with nobody asking it to', async () => {
    ...
    overlay.render([issueOnCta()]);
    const before = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
    assert.equal(before.dataset.fruitStrategy, 'selector');
  ...
  it('stops claiming to be sure when the element it recognised is gone', async () => {
    ...
    const pin = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
    assert.equal(pin.dataset.fruitConfident, 'true');
    ...
    assert.equal(pin.dataset.fruitConfident, 'false');
    assert.equal(pin.querySelector('.fruit-pin-glyph')?.textContent, '≈');
    assert.ok(pin.classList.contains('fruit-pin-uncertain'));
  });
  ...
  it('does not close a thread someone is reading', async () => {
    ...
    overlay.render([issueOnCta()]);
    (page.document.querySelector('.fruit-pin-badge') as HTMLElement).click();
    assert.equal(page.document.querySelectorAll('[data-fruit-thread]').length, 1);

    page.document.querySelector('main')?.append(page.document.createElement('div'));
    await settle();

    assert.equal(page.document.querySelectorAll('[data-fruit-thread]').length, 1, 'still open');
  });
...
describe('showing only some pins', () => {
  it('draws only what the filter accepts, and can change its mind without new data', () => {
    ...
    assert.equal(page.document.querySelectorAll('[data-fruit-pin]').length, 2);

    hidden = ['ripe'];
    overlay.refilter();

    const stages = [...page.document.querySelectorAll('[data-fruit-pin]')].map(
      (pin) => (pin as HTMLElement).dataset.fruitStage,
    );
...
describe('the issues it was handed', () => {
  it('keeps a copy, so a caller mutating their array does not change the screen', () => {
    ...
    assert.equal(page.document.querySelectorAll('[data-fruit-pin]').length, 1);
  });
});
...
describe('the team’s replies', () => {
  it('shows them oldest first, with who wrote each', () => {
    ...
    const bodies = [...page.document.querySelectorAll('.fruit-thread-reply-body')].map((node) => node.textContent);
    assert.deepEqual(bodies, ['On regarde ça.', 'Corrigé sur la préprod.']);
    assert.match(page.document.querySelector('.fruit-thread-reply-who')?.textContent ?? '', /Alice/);
  });
  ...
  it('renders a reply as text, never as markup', () => {
    ...
    const body = page.document.querySelector('.fruit-thread-reply-body');
  ...
  it('says nothing at all when the worker did not fetch replies', () => {
    ...
    assert.equal(page.document.querySelector('.fruit-thread-empty'), null);
    assert.equal(page.document.querySelector('.fruit-thread-replies'), null);
  });
  ...
  it('says so when it asked and there were none', () => {
    ...
    assert.match(page.document.querySelector('.fruit-thread-empty')?.textContent ?? '', /Pas encore/);
  });
});
...
describe('the detached notes', () => {
  ...
  it('stays out of the way when every pin found its element', () => {
    ...
    assert.equal((page.document.querySelector('[data-fruit-orphans]') as HTMLElement).hidden, true);
  });
  ...
  it('lists a note whose element the cascade could not find at all', () => {
    ...
    const drawer = page.document.querySelector('[data-fruit-orphans]') as HTMLElement;
    assert.equal(drawer.hidden, false);
    assert.match(drawer.querySelector('.fruit-orphans-toggle')?.textContent ?? '', /1 note détachée/);
    assert.match(
      drawer.querySelector('.fruit-orphans-note')?.textContent ?? '',
  ...
  it('does not list a pin that was placed by position', () => {
    ...
    const listed = page.document.querySelectorAll('.fruit-orphans-item').length;
    const pin = page.document.querySelector('[data-fruit-pin]') as HTMLElement;
  ...
  it('empties and hides itself once the element is back', () => {
    ...
    assert.equal((page.document.querySelector('[data-fruit-orphans]') as HTMLElement).hidden, false);

    overlay.render([issueOnCta()]);

    const drawer = page.document.querySelector('[data-fruit-orphans]') as HTMLElement;
    assert.equal(drawer.hidden, true);
    assert.equal(drawer.querySelectorAll('.fruit-orphans-item').length, 0);
  });
  ...
  it('opens the note when its entry is clicked', () => {
    ...
    (page.document.querySelector('.fruit-orphans-note') as HTMLElement).click();
  ...
  it('takes its DOM with it when the overlay is destroyed', () => {
    ...
    assert.equal(page.document.querySelector('[data-fruit-orphans]'), null);
  });
});
