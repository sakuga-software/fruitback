...
describe('the token declarations', () => {
  ...
  it('declares every settable token in the base block, not only under a media query', () => {
    const undeclared = THEME_TOKENS.filter((token) => !baseBlock.includes(`--fruitback-${token}:`));
  ...
  it('has a token for every stage the contract names', () => {
    ...
    for (const stage of SEED_STAGES) {
      assert.ok(baseBlock.includes(`--fruitback-stage-${stage}:`), `${stage} has no unconditional declaration`);
      assert.equal(stageToken(stage), `var(--fruitback-stage-${stage})`);
    }
  });
  ...
  it('redefines colours for a dark scheme, and durations in neither', () => {
    ...
    assert.ok(dark.includes('--fruitback-color-surface:'), 'dark does not restate the surface colour');
    assert.ok(!dark.includes('--fruitback-duration-'), 'a duration has no business changing with the scheme');
  });
...
describe('applyTheme', () => {
  ...
  it('writes the tokens a host asked for', () => {
    ...
    assert.equal(element.style.getPropertyValue('--fruitback-color-accent'), '#0055ff');
    assert.equal(element.style.getPropertyValue('--fruitback-stage-ripe'), 'rebeccapurple');
  });
  ...
  it('ignores a name it does not know, rather than writing it', () => {
    ...
    assert.equal(element.style.getPropertyValue('--fruitback-colour-accent'), '');
    assert.equal(element.style.getPropertyValue('--fruitback-position'), '');
...
describe('a foreground token is only ever used on the background it is named for', () => {
   ...
   * (`--fruitback-color-chip`), every pin badge (`--fruitback-pin-color`) and the orphan chip
   * (`--fruitback-color-warning`). Nothing looked wrong, because all four tokens hold `#fff` — a host
  ...
  const PAIRS: Record<string, string> = {
    'on-accent': '--fruitback-color-accent',
    'on-chip': '--fruitback-color-chip',
    'on-stage': '--fruitback-pin-color',
    'on-warning': '--fruitback-color-warning',
  };
  ...
  it('finds every foreground token beside its paired background', async () => {
    ...
    for (const { where, body } of blocks) {
      for (const [token, background] of Object.entries(PAIRS)) {
        if (!body.includes(`var(--fruitback-color-${token})`)) continue;
